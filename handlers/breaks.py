...
# --- State Definitions ---
# Ensure these match the states in your main.py
...
async def send_warning_callback(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    It retrieves the chat_id and message from the job's data payload.
    """
    job = context.job
    try:
...
# --- *** REWRITTEN AND FIXED end_break FUNCTION *** ---
async def end_break(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles the end of any break. This version is fixed to prevent crashes."""
    user = update.effective_user
    now = get_current_time()

    # Immediately cancel any pending warning for this user
    await _remove_previous_job(user.id, context)

    # Safely get data from user_data
    break_start_time = context.user_data.get('break_start_time')
    break_type = context.user_data.get('current_break_type', 'unknown')

    if not context.user_data.get('on_break') or not break_start_time:
        await update.message.reply_text(
            "It seems you weren't on a break. Returning to the main menu.",
    ...
    late_message = ""
    if break_type == 'toilet' and duration_seconds > TOILET_BREAK_LIMIT_SECONDS:
        over_by = duration_seconds - TOILET_BREAK_LIMIT_SECONDS
        late_message = f"\n🚨 *You were late by {format_duration(over_by)}!* 🚨"
        log_activity(user, f'{break_type}_overtime', f"Exceeded by {format_duration(over_by)}")
    ...
    # --- IMPORTANT: Reset user's state BEFORE sending the final message ---
    # This prevents the bot from getting stuck if the keyboard function fails.
    context.user_data['on_break'] = False
    ...
    # --- Build and send the final report message ---
    response_message = (
        f"👤 *User:* {user.full_name}\n🆔 *User ID:* {user.id}\n"
        f"------------------------------------\n"
        ...
        f"------------------------------------\n"
        f"Counts: 🍔{context.user_data.get('eat_breaks_today', 0)} | "
        f"🚽{context.user_data.get('toilet_breaks_today', 0)} | "
        f"🛌{context.user_data.get('rest_breaks_today', 0)}"
        f"{late_message}"
    )

    try:
        # Now it is safe to generate the keyboard
        reply_markup = ReplyKeyboardMarkup(main_keyboard(context.user_data), resize_keyboard=True)
        await update.message.reply_text(response_message, reply_markup=reply_markup, parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Failed to send end_break message or keyboard: {e}")
        # Send a fallback message if the keyboard fails
        await update.message.reply_text("Your break has ended. Use /start to see the main menu.")
